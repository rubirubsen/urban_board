...
export const TelemetryWidget: React.FC<TelemetryWidgetProps> = ({
  ...
  return (
    <div className="space-y-4 font-mono text-xs w-full max-w-full overflow-x-hidden">
      ...
      <div className="p-3 rounded bg-anthrazit-900 border border-anthrazit-800 space-y-2.5">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-1.5 text-anthrazit-300">
            ...
            <span className="font-bold text-[11px] text-anthrazit-200 uppercase truncate">
              {weather.station}
            </span>
          </div>
          <button
            ...
            className="p-1 rounded hover:bg-anthrazit-800 text-anthrazit-400 hover:text-accent cursor-pointer transition-colors"
        ...
        <div className="grid grid-cols-2 gap-2 text-anthrazit-100">
          <div className="p-2 rounded bg-anthrazit-950 border border-anthrazit-850">
            <div className="text-[10px] text-anthrazit-400 uppercase">Temperatur</div>
            <div className="text-xl font-bold font-mono text-accent">
              {weather.temp}°C
            </div>
            <div className="text-[10px] text-anthrazit-400 truncate">{weather.condition}</div>
          </div>

          <div className="p-2 rounded bg-anthrazit-950 border border-anthrazit-850">
            <div className="text-[10px] text-anthrazit-400 uppercase">Luftfeuchte</div>
            <div className="text-xl font-bold font-mono text-sky-400">
              {weather.humidity}%
            </div>
            <div className="text-[10px] text-anthrazit-400">rel. Feuchte</div>
          </div>

          <div className="p-2 rounded bg-anthrazit-950 border border-anthrazit-850">
            <div className="text-[10px] text-anthrazit-400 uppercase">Wind (Böen)</div>
            <div className="text-sm font-bold font-mono text-anthrazit-200">
              {weather.windSpeed} km/h
            </div>
            <div className="text-[10px] text-accent">Böen: {weather.windGust} km/h</div>
          </div>

          <div className="p-2 rounded bg-anthrazit-950 border border-anthrazit-850">
            <div className="text-[10px] text-anthrazit-400 uppercase truncate">{pegelLabel}</div>
            <div className="text-sm font-bold font-mono text-emerald-400">
              {livePegel} m
            </div>
            <div className="text-[10px] text-anthrazit-500">{pegelSource}</div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="space-y-2 max-h-96 overflow-y-auto">
          {liveSenseBoxes.map((box) => (
            <div
              key={box.id}
              className="p-3 rounded bg-anthrazit-900 border border-anthrazit-800 hover:border-accent transition-colors"
            >
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-[10px] px-1.5 py-0.5 rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 font-bold shrink-0">
                  {box.lastMeasurement}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-1.5 text-[11px]">
                {box.sensors.map((s, idx) => (
                  <div key={idx} className="p-1.5 rounded bg-anthrazit-950 border border-anthrazit-850 flex items-center justify-between">
