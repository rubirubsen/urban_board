...
interface TrafficFeedProps {
  ...
  activeCity?: 'H' | 'HH';
  selectedStation?: TransitStop | null;
  onSelectStation?: (station: TransitStop | null) => void;
}
...
export const TrafficFeed: React.FC<TrafficFeedProps> = ({
  ...
  // Only hold activeStation when explicitly chosen (null by default)
  const [activeStation, setActiveStation] = useState<TransitStop | null>(selectedStation);
  ...
  // Sync with prop when selectedStation changes from outside (e.g. Map click or Hub click)
  useEffect(() => {
    if (selectedStation) {
  ...
  // Reset station when city switches so no arbitrary departures are shown per se
  useEffect(() => {
  ...
  // Only load departures when an activeStation is explicitly selected
  useEffect(() => {
    if (!activeStation) {
      setDepartures([]);
      return;
  ...
  const isCurrentActivePinned = activeStation
    ? pinnedStops.some(
        s => s.id === activeStation.id || s.name.trim().toLowerCase() === activeStation.name.trim().toLowerCase()
      )
    : false;
          ...
          <div className="p-3 rounded bg-anthrazit-900 border border-anthrazit-800 space-y-2.5">
            ...
            <div ref={searchContainerRef} className="relative">
              <div className="flex items-center bg-anthrazit-950 border border-anthrazit-700 rounded px-2 py-1.5 focus-within:border-accent min-h-[44px]">
                ...
                <input
                  type="text"
                  placeholder="Haltestelle suchen (z. B. Fenskestraße, Dammtor)..."
                  value={searchQuery}
              ...
              {pinnedStops.length === 0 ? (
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  {pinnedStops.map((stop) => {
                    const isSelected = activeStation && (
                      activeStation.id === stop.id || 
                      activeStation.name.trim().toLowerCase() === stop.name.trim().toLowerCase()
                    );
                    return (
                      <div
                        key={stop.id}
              ...
              <div className="pt-2 border-t border-anthrazit-800/70 flex items-center justify-between text-xs">
                ...
                <div className="flex items-center space-x-1.5 shrink-0">
                  <button
                    ...
                    title={isCurrentActivePinned ? 'Aus Favoriten entfernen' : 'Haltestelle als Favorit merken'}
                  >
                    <Pin className={`w-3 h-3 ${isCurrentActivePinned ? 'fill-current' : ''}`} />
          ...
          {activeStation ? (
            <div className="space-y-1.5">
              ...
              {loadingDepartures && departures.length === 0 ? (
                ...
                <div className="p-4 rounded bg-anthrazit-900 border border-anthrazit-800 text-center text-anthrazit-400">
            ...
            <div className="p-4 rounded bg-anthrazit-900 border border-anthrazit-800 text-center space-y-2">
              <div className="w-8 h-8 rounded-full bg-accent/15 border border-accent/30 text-accent flex items-center justify-center mx-auto">
